...
<img width="576" height="421" alt="20210801130546" src="https://user-images.githubusercontent.com/74708028/127759004-305bf99f-1655-490d-9fa4-4c863fe5eb5e.png">
...
* `PostDetailProc`에서 request.setAttribute하여 jsp page로 넘겨줄 값의 목록은 다음과 같습니다. 
   * ##### 넘어온 페이지로부터 게시물 고유번호(post_idx)에 해당하는 글 
   * ##### 해당 글에 저장한 이미지
   * ##### 페이지 사이드에 위치한 `최근 글 목록`에 보여줄 글 목록 
   * ##### 페이지 하단에 위치한 `관련글`에 뿌려줄 글 목록
   * ##### 해당 글의 카테고리명
   
  <br>
  
✔ 이 중 포인트는 넘어온 페이지로부터 게시물 고유번호(post_idx)를 얻는 것이었습니다. 
* request가 어디서 넘어왔냐에 따라 상세 페이지에서 보여줄 데이터가 달라지기 때문이었습니다. 
